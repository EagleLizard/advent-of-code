...
pub fn get_hand_type(cards: &[PlayingCard; 5], with_joker: bool) -> PlayingHand {
  ...
  */
  let mut card_counts = get_card_counts(cards);
  // println!("{:#?}", card_counts);
  ...
  let max_card_key = get_max_card_key(&card_counts);
  let max_card_value = card_counts.get(&max_card_key).unwrap();
  
  
  card_counts.insert(max_card_key, max_card_value + joker_count).unwrap();
  let max_card_val = card_counts.get(&max_card_key).unwrap();
...
fn get_max_card_key<'a>(
  card_counts: &'a HashMap<PlayingCard, u8>
)  -> PlayingCard {
  let max_card_pair = card_counts.iter().reduce(|acc, curr| {
    if acc.1 > curr.1 {
      acc
    } else {
      curr
  ...
  *max_card_pair.to_owned().0
}

fn get_card_counts(cards: &[PlayingCard; 5]) -> HashMap<PlayingCard, u8> {
